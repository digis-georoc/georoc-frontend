...
<template>
  <DataTable :value="rows" :paginator="true" :rows="10" :rowsPerPageOptions="[5, 10, 15, 20]" 
    paginatorTemplate="CurrentPageReport FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink RowsPerPageDropdown" 
    :currentPageReportTemplate="t('showing_first_to_last_of_total_entries', {first: '{first}', last: '{last}', totalRecords: '{totalRecords}'})"
    :removableSort="true"
    ...
    :pt="{
      ...
      paginator: {
        root: {
          class: 'flex flex-wrap items-center justify-center space-x-2 text-gray-500 border-t-2 px-4 py-2 dark:text-white/60'
        },
    ...
    <template #header>
      ...
      :pt="{
        root: {
          class: 'p-1 border-2 border-transparent rounded-md bg-transparent transistion-color duration-200 hover:border-primary'
        }  
      }"></InputText>
    </template>
    <template #empty>{{ $t('no_matching_records_found') }}</template>
    <template #paginatorfirstpagelinkicon><Icon :name="'prime:angle-double-left'" class="min-w-[1.5rem] min-h-[1.5rem]"/></template>
    <template #paginatorprevpagelinkicon><Icon :name="'prime:angle-left'" class="min-w-[1.5rem] min-h-[1.5rem]"/></template>
    <template #paginatornextpagelinkicon><Icon :name="'prime:angle-right'" class="min-w-[1.5rem] min-h-[1.5rem]"/></template>
    <template #paginatorlastpagelinkicon><Icon :name="'prime:angle-double-right'" class="min-w-[1.5rem] min-h-[1.5rem]"/></template>
    <template #paginatorrowsperpagedropdownicon><Icon :name="'prime:angle-down'" class="min-w-[1.5rem] min-h-[1.5rem]"/></template>
