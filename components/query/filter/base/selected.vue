...
<template>
  <div class="flex flex-wrap gap-1.5" v-if="items && items.length > 0">
    ...
    <BasePopover
      button-styles="bg-gray-100 dark:bg-gray-600 py-1.5 px-2 text-primary dark:text-gray-400 font-bold rounded-lg flex items-center flex-shrink-0 text-sm leading-none"
    ...
    >
      ...
      <div class="min-w-[160px] max-h-[300px] overflow-auto flex flex-col items-start gap-1.5">
        <div class="bg-gray-100 dark:bg-gray-700 py-1.5 px-2 text-gray-500 dark:text-gray-400 font-bold rounded-lg flex items-center" v-for="(item, i) in hiddenItems">
          <span class="text-sm leading-none">{{ $t(item.label) }}</span>
