...
async function loadChildren(node: TreeNode) {
  ...
  const rockClasses = await getRockClasses(nodes.value[index].key)
  nodes.value[index].children = rockClasses
    ?.data
    .map(({ value, label }) => ({ key: generateChildKey(node.key, value), label: label ?? value }))
    ?? []
