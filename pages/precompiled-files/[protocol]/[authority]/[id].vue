  ...
  <PageContainer>
    ...
    <div v-else>
      <section>
        ...
        <div
          class="sticky top-[4rem] bg-white dark:bg-zinc-800 z-[1009] py-2 border-b-2"
        >
          <div
        ...
        <BaseFluidContainer
          class="mb-6 mt-4 py-4 bg-primary-50 dark:bg-zinc-700 rounded-md"
        >
          <div class="flex space-x-3">
            <Icon
              :name="'system-uicons:document-stack'"
              class="w-[10rem] h-[8rem]"
            ></Icon>
            <div class="space-y-5">
              <p class="font-bold">{{ authorString }}</p>
              <p>
                {{ precompiledFiles?.title }},
                <NuxtLink
                  class="border-b-primary hover:border-b-2 text-primary dark:text-primary-100"
                  :to="precompiledFiles?.persistentUrl"
                  >{{ precompiledFiles?.persistentUrl }}</NuxtLink
                >
              </p>
              ...
              <p>
                <button
                  ...
                  "
                >
                  Download complete precompiled dataset
                </button>
              </p>
            </div>
          </div>
            ...
            <BaseFluidContainer class="py-4">
              ...
              >
                <template #[file.label] v-for="file in precompiledFiles?.files">
                  <button
                    class="underline underline-offset-4 text-primary dark:text-primary-100"
                    @click="openDialog(file.persistentId, false, file.label)"
                  >
                    {{ file.label }}
                  </button>
                </template>
      ...
      <BaseFluidContainer
        :class="[
          'pt-4 pb-6 space-y-4 snap-y',
          { 'border-t-2': !$slots.default },
        ]"
      >
        ...
        <h3 class="text-lg font-semibold">
          {{ $t('precompiledFilePage.subheadline_2') }}:
        </h3>
        ...
        <h3 class="text-lg font-semibold">
          {{ $t('precompiledFilePage.subheadline_3') }}:
        </h3>
        <ul class="list-disc list-inside ml-6">
        ...
        <br />
        <h3 class="text-lg font-semibold">
          {{ $t('precompiledFilePage.subheadline_4') }}:
        </h3>

        <h4 class="font-medium">
          {{ $t('precompiledFilePage.subsubheadline_1') }}:
        </h4>
        <ul class="list-disc list-inside ml-6">
        ...
        <h4 class="font-medium">
          {{ $t('precompiledFilePage.subsubheadline_2') }}:
        </h4>
        <ul class="list-disc list-inside ml-6">
        ...
        <h4 class="font-medium">
          {{ $t('precompiledFilePage.subsubheadline_3') }}:
        </h4>
        <ul class="list-disc list-inside ml-6">
          <li>{{ $t('precompiledFilePage.tims') }}</li>
          <li>{{ $t('precompiledFilePage.icpms') }}</li>
  ...
  },
  {
    subtitle:
      precompiledFiles && precompiledFiles?.authors.length > 1
        ? t('precompiledFilePage.meta_data_4_pl')
        : t('precompiledFilePage.meta_data_4_sing'),
    data: authorString,
  },
  {
...
async function onSubmit() {
  ...
  await download(
    ...
    downloadOptions.isDataset,
    downloadOptions.filetitle,
  )
