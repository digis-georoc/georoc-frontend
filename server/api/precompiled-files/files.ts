...
export default defineEventHandler<PrecompiledFiles | H3Error | undefined>(async ({ node }: H3Event) => {
    try {
        const params = new URLSearchParams(node.req.url?.split('?')[1]);
        const paramsObj = Object.fromEntries(params.entries());
        const data = (await fetchData(`datasets/:persistentId/?persistentId=${paramsObj['protocol']}:${paramsObj['authority']}/${paramsObj['id']}`))["data"];
        ...
        const precomp: PrecompiledFiles = {
            ...
            description: citation["fields"][3]["value"][0]["dsDescriptionValue"]["value"],
